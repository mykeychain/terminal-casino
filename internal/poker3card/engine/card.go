...
// Ace high (14). Straight logic treats the Ace as either high or low.
type Rank int
...
// Name returns the full singular rank name (e.g. "King", "Ace", "Two"), used
// by the hand evaluator to build human-readable descriptions.
...
// Deck is an ordered set of 52 cards. Three-Card Poker uses one freshly
// shuffled 52-card deck per hand (no shoe, no cut card).
type Deck struct {
	cards []Card
	pos   int
	rng   *rand.Rand
