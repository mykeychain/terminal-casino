...
// (narrow terminals) drops it so all six cards still fit.
...
// renderHandCount lays out the first `count` cards of a hand as side-by-side
// face-up cards. The feel tier passes a count below len(cards) so the initial
// deal can cascade in one card at a time.
...
// renderDealerHandFrame lays out the dealer hand from a revealFrame: it draws
// f.dealerCards slots, of which [0,dealerFaceUp) are face up (read from dv.Cards)
// and the rest are face-down backs. Before the reveal the dealer view carries no
// cards, so the hidden slots draw from a fixed count rather than from dv.Cards —
// this is what lets three backs sit on the felt through the decision phase and
// then flip one at a time at showdown.
func renderDealerHandFrame(dv engine.DealerView, f revealFrame, compact, short bool) string {
	boxes := make([]string, 0, f.dealerCards)
	for i := 0; i < f.dealerCards; i++ {
		if i < f.dealerFaceUp && i < len(dv.Cards) {
