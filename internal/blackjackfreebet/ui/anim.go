...
func (m Model) renderBanner() string {
	...
	if len(hands) == 1 {
		...
		case engine.OutcomePush:
			text, style = pushHeadline(m), pushStyle
...
// pushHeadline names a push, calling out the Push 22 rule when the dealer's tie
// came from a 22 rather than an equal total.
func pushHeadline(m Model) string {
	if m.game.DealerPush22() {
		return "PUSH · DEALER 22"
	}
	return "PUSH"
}
